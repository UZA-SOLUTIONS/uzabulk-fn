...
export default function BestDealsDiscover() {
  ...
  logger("Best deals discover ::: ", items);
  const limit = 100;
  const navigate = useNavigate();
  const filteredBestDeals = (items || []).filter((item) => {
    const name = (item?.name || "").toLowerCase().trim();
    return name && !name.includes("test");
  });
  const filteredTopRanked = (topRankedItems || []).filter((item) => {
    const name = (item?.name || "").toLowerCase().trim();
    return name && !name.includes("test");
  });
  ...
  }, [dispatch, topRankedItems?.length]);
      ...
      {isLoading || isTopRankedLoading ? (
        ...
        <div className="discover_card home_feed_section_offset px-3 h-100 w-100">
          <div className="card_top_head d-flex align-items-center justify-content-center">
            <h4 className="home_section_title_line">You may also like</h4>
            <Link to={ROUTES.TOP_RANKING_PRODUCT_LISTING} className="home_section_view_more">
              View More
            </Link>
          </div>
                    ...
                    >
                      <div className="new_arrival_media">
                        <img
                          ...
                          alt={item?.name || "Best deal product"}
                          className="img-fluid"
                        />
                      </div>
